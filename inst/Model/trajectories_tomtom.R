# Tom Tom trajectories
trajectories_tomtom <- function(origin, dest, mode, hour_trajectory, key){
  num_alternative <- 5
  hour_trajectory <- as.POSIXct(strptime(hour_trajectory, 
                                         format = "%Y-%m-%d %H:%M:%S"))
  ...
  if(substr(zone_tot, 1, 1) == "-"){
    ...
    hour_format <- paste(day, "T", hours, "%3A", minutes, "%3A00", zone, 
                         "%3A00", sep = "")
  }else{
    zone <- substr(format(hour_trajectory, "%z"), 2, 3)
    hour_format <- paste(day, "T", hours, "%3A", minutes, "%3A00%2B", zone,
                         "%3A00", sep = "")
  }
  df_rbind <- data.frame()
  ...
  url<- paste0("https://api.tomtom.com/routing/1/calculateRoute/", origin_lat,
               "%2C", origin_long, "%3A", destination_lat, "%2C", 
               destination_long, "/json?maxAlternatives=", 
               num_alternative, "&departAt=", hour_format, 
               "&routeRepresentation=polyline&computeTravelTimeFor=all&traffic=true&travelMode=",mode,"&vehicleEngineType=combustion&key=",key)
  ...
  for (j in 1:length(resp_json[["routes"]][["legs"]])){
    resp<- data.frame( long = resp_json[["routes"]][["legs"]][[j]][["points"]]
                       [[1]][["longitude"]],
                       
                       lat = resp_json[["routes"]][["legs"]][[j]][["points"]]
                       [[1]][["latitude"]],
                       
                       #Arrival and departure time
                       departureTime = resp_json[["routes"]][["legs"]][[j]]
                       [["summary"]][["departureTime"]],
                       
                       arrivalTime = resp_json[["routes"]][["legs"]][[j]]
                       [["summary"]][["arrivalTime"]],
                       
                       #Distance 
                       lengthInKM = (resp_json[["routes"]][["legs"]][[j]]
                                     [["summary"]][["lengthInMeters"]] / 1000),
                       trafficLengthInKM= (resp_json[["routes"]][["legs"]][[j]]
                                           [["summary"]][["trafficLengthInMeters"]]/
                                             1000),
                       travelMode=resp_json[["routes"]][["sections"]][[1]]
                       [["travelMode"]][1],
                       ...
                       travelTimeInMinutes = round((resp_json[["routes"]]
                                                    [["legs"]][[j]][["summary"]]
                                                    [["travelTimeInSeconds"]]/
                                                      60), 2),
                       liveTrafficIncidentsTravelTimeInMinutes = 
                         round((resp_json[["routes"]][["legs"]][[j]]
                                [["summary"]]
                       ...
                       noTrafficTravelTimeInMinutes =
                         round((resp_json[["routes"]][["legs"]][[j]][["summary"]]
                                [["noTrafficTravelTimeInSeconds"]]/60),2),
