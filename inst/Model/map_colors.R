...
# This function is used in the 'tom-tom' function and in the 'total exposure' 
#function.
# It requires inputting a grid with pollutant information as well as the name of
#the pollutant. 
# As output, it provides the same input grid with a new column containing AQI 
#information
...
map_colors <- function(grid, pollutant){ 
  if (pollutant == "PM2.5"){
    ...
    grid$category = dplyr::case_when(grid$value <= 12.1 ~ 'Good',                           
                              grid$value > 12.1 & grid$value <= 35.4  ~ 
                                'Moderate', 
                              grid$value > 35.4 & grid$value <= 55.4  ~ 
                                'Unhealthy for sensible groups', 
                              grid$value > 55.4 & grid$value <= 150.4  ~ 
                                'Unhealthy', 
    ...
    grid$color = dplyr::case_when(grid$category == 'Good' ~ "#abdda4",                           
                           ...
                           grid$category == 'Unhealthy for sensible groups' ~ 
                             "#fdde61",
    ...
    grid$category = dplyr::case_when(grid$value <= 4.4 ~ 'Good',
                              ...
                              grid$value > 9.4 & grid$value <= 12.4  ~ 
                                'Unhealthy for sensible groups',
                              grid$value > 12.4 & grid$value <= 15.4  ~ 
                                'Unhealthy',
    ...
    grid$category = dplyr::case_when(grid$value <= 35 ~ 'Good',
                              ...
                              grid$value > 185 & grid$value <= 304  ~ 
                                'Unhealthy', ##4 1-hour SO2 values do not define 
                              #higher AQI values (≥ 200). AQI values of 200 or 
                              #greater are calculated with 24-hour SO2 concentrations.
                              grid$value > 304 & grid$value <= 604  ~ 
                                'Very unhealthy', #4 1-hour SO2 values do not 
                              #define higher AQI values (≥ 200). AQI values of 
                              #200 or greater are calculated with 24-hour SO2 
                              #concentrations.
                              grid$value > 604 ~ 'Hazardous' ) #4 1-hour SO2 
                              #values do not define higher AQI values (≥ 200). 
                              #AQI values of 200 or greater are calculated with 
