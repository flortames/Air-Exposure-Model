temporary_grid_search <- function(start_hour, end_hour=NULL, dir, time_format, 
                                  gridID, shapeValue){  
 
  # --- Function that looks for the grid (.shp) corresponding to the hour of 
  #interest entered
  
  trajectory_grid_rbind <- data.frame()
  
  # Init time
  only_start_hour <- lubridate::hour(as.POSIXct(strptime(start_hour, format = 
                                                           time_format)))
  # End time
  only_end_hour <- lubridate::hour(as.POSIXct(strptime(end_hour, format = 
                                                         time_format)))
  if (is.null(end_hour)){
    df_start_grids <- sf::st_read(hourly_grid(hour = start_hour, time_format = 
                                                time_format, dir), quiet = TRUE)
    df_start_grid <- sf::st_transform(df_start_grids, crs = 4326)
    
    # Always transform the CRS in 4326
  ...
  #  --- When there is only one grid
    ...
    trajectory_grid <- sf::st_read(hourly_grid(start_hour, time_format = 
                                                 time_format, dir), quiet = TRUE)
    ...
    for(j in only_start_hour:only_end_hour){
      ...
      day <- paste(substr(start_hour, 1, 10), paste(j_hour, ":00:00", sep = ""), 
                   zone, sep = " ")
      trajectory_grid <- sf::st_read(hourly_grid(day, time_format = 
                                                   "%Y-%m-%d %H:%M:%S",dir), 
                                     quiet = TRUE)
    ...
    for (p in 1:length(data_grilla)){
      ...
      df <- data.frame(ID = ID, value = value, geometry = geometry)
      df_grilla <- rbind(df_grilla, df)
    }
    sf::st_write(df_grilla, "./temp/temp_grid.shp", delete_layer = TRUE, 
                 quiet = TRUE)
