...
total_exposure <- function (travel_list, mode, dir, key, selection, output_exp, 
                            departure_time_home, activity_minutes, pollutant, 
                            shapeValue, gridID, units){
  ...
  for (i in 1:length(travel_list$long)){
    ...
    if(i == 1){
      ...
      destination_coords <- paste((travel_list[i + 1, 2]), (travel_list[i + 1, 1]), 
                                  sep = ",")
      selection_route <- alternative_trajectories(origin = origin_coords, dest 
                                                  = destination_coords, mode = 
                                                    mode[i], dir, key = key, 
                                                  output = "df", hours = 
                                                    departure_time_home, gridID, 
                                                  shapeValue, units)
    }
    else if (i == length(travel_list$long)){
      # If the coordinates correspond to the last point entered, destination 
      #returns to point 1 (home)
      origin_coords <- paste((travel_list[i, 2]), (travel_list[i, 1]), sep = ",")
      origin_coords_1 <- paste((travel_list[1, 2]), (travel_list[1, 1]), sep = ",")
      selection_route <-  alternative_trajectories (origin = origin_coords, 
                                                    dest = origin_coords_1, mode 
                                                    = mode[i], dir, key = key, 
                                                    output = "df", hours = 
                                                      prox_hour_output, gridID, 
                                                    shapeValue, units)
    }else {
      origin_coords <- paste((travel_list[i, 2]), (travel_list[i, 1]), sep = ",")
      destination_coords <- paste((travel_list[i + 1, 2]), (travel_list[i + 1, 1]), 
    ...
    # ------- hours that we use to get the destination grid
     if(i==1){
      ...
      arrival_time <- as.POSIXct(strptime(selection_route$arrivalTime[1], 
                                           format = "%Y-%m-%dT%H:%M:%S"))
      prox_hour_output <- as.POSIXct(strptime(selection_route$arrivalTime[1], 
                                              format = "%Y-%m-%dT%H:%M:%S")) + 
        minutes(activity_minutes[i, ])
      destination_time <- as.numeric(difftime(prox_hour_output, arrival_time, 
                                              unit = "mins"))
    }
    
    else if (i == nrow(activity_minutes) + 1){
      # If it is the last entered time ==> entered time - 23:59
      prox_hour_output <- as.POSIXct(strptime((paste(substr(departure_time_home, 
                                                            1, 10)," 23:59:59", 
                                                     sep = "")), format = 
                                                "%Y-%m-%d %H:%M:%S"))
      arrival_time <- as.POSIXct(strptime(selection_route$arrivalTime[1], format 
                                          = "%Y-%m-%dT%H:%M:%S"))
      destination_time <- as.numeric(difftime(prox_hour_output, arrival_time, 
                                              unit = "mins"))
    }else{
    ...
    df_concentractions <- st_as_sf(df_concentractions, coords = c("long", "lat"), 
                                   crs = 4326)
    dir_point <- paste (dir, "/temp/", sep = "")
    name_point <- paste(dir_point, "temp_point.shp", sep = "")
    st_write(df_concentractions, paste(dir_point, "temp_point.shp", sep = ""), 
             driver = "ESRI Shapefile", quiet = TRUE)
    point <- st_read(name_point, quiet = TRUE)
    
    # Function that looks for the grids of the hours of interest
    grid <- temporary_grid_search(start_hour = arrival_time, end_hour = 
                                     prox_hour_output, dir = dir, time_format = 
                                     "%Y-%m-%d %H:%M:%S", gridID, shapeValue)
    intersection_point <- sf::st_intersection(point, grid)
    names(intersection_point) <- c("altrntv", "ID", "dprtrTm", "arrvlTm", 
                                   "lngthIM", "trffLIM", "travlMd", "trffDIS", 
                                   "trvlTIS", "lTITTIS",  "hsTTTIS",  "nTrTTIS", 
                                   "value", "exposur", "type", "i", "gridID", 
                                   "value.1", "geometry")
    ...
    if (i == 1){
      ...
      start_time <- paste (substr(arrival_time, 1, 10), "00:00:01 ", zone, 
                           sep = " ")
      grid_origin <- temporary_grid_search (start_hour = start_time, end_hour = 
                                              departure_time_home, dir = dir, 
      ...
      time_origin <- as.numeric(difftime(departure_time_home, start_time, unit 
                                         = "mins"))
    }else{
      conc_origin <- NA
      time_origin <- NA
  ...
  rbind_df_1$exp_tot_destination <- rbind_df_1$conc_destination * 
  ...
  exp_tot <- round((mean(c(exp_origin, exp_destination, exp_trajectory), na.rm = 
                           T)/60), 2)
  ...
  exp_tot2 <- round((sum(c(exp_origin2, exp_destination2, exp_trajectory2), 
                         na.rm = T)/60), 2)
  rbind_df_1$exp_tot_sum <- exp_tot2
  rbind_df_1$route <- paste("Route", rbind_df_1$i, sep= " " )
  rbind_df_1$mode <- mode
  time_origin_function <- function_hours(sum(rbind_df_1$destination_time
                                             [max(rbind_df_1$i)] + rbind_df_1$
                                               time_origin[!is.na(rbind_df_1$
                                                                    time_origin)]))
  trip_time_function <- function_hours((sum(rbind_df_1$trip_time)))
  destination_time_function <- function_hours(sum(rbind_df_1$destination_time
                                                  [1:(length(rbind_df_1$
                                                               destination_time) 
                                                      - 1)]))
  
  #  --- Point to line transformation
  route_line <- points_to_line(data = rbind_route_select, long = "long", lat = 
                                 "lat", id_field = "i", sort_field = "ID")
...
"))
  ...
  title <-  htmltools::tags$div(tag.map.title, HTML(paste(sep = "<br/>", 
                                              paste0("<center><b>Total daily 
                                                     exposure estimate</b></center>"),
                                              paste0("<b>Exposure: </b>", exp_tot2, 
                                                     " ", units, "/ 24hs"),
                                              paste0("<b>Origin time: </b>", 
                                                     time_origin_function, "hs"),
                                              paste0("<b>Activities time: </b>", 
                                                     destination_time_function, 
                                                     " hs"),
                                              paste0("<b>Travel time: </b>",  
                                                     trip_time_function, "hs"))))
  #print(title)
  #  --- Travel list ID
  num_rows <- nrow(travel_list)
  id <- c(1:num_rows)
  travel_list <- cbind(id, travel_list)
  input_hour <- as.POSIXct(strptime(departure_time_home, format = 
                                      "%Y-%m-%d %H:%M:%S"))
  init_day <- paste(date(input_hour), "00:00:01", sep= " ")
  finish_day <- paste(date(input_hour), "23:59:01", sep= " ")
  grid_tot <- temporary_grid_search (start_hour = init_day, end_hour = finish_day, 
                                     dir = dir, time_format = "%Y-%m-%d %H:%M:%S", 
                                     gridID, shapeValue)
  ...
  #  --- Color grid
  palette_route <- c("#023858", "#49006a", "#00441b", "#e7298a", "#feb24c", 
                     "#3690c0", "#016c59", "#8c510a", "#f03b20")
  ...
  mapa <- leaflet::leaflet() %>%
    leaflet::addTiles() %>%
    leaflet::addAwesomeMarkers(travel_list, lng=travel_list$long, lat = 
                                 travel_list$lat, label = paste("Point", 
                                                                travel_list$id, 
                                                                sep = " ")) %>%
    leaflet::addPolylines(data = route_line, opacity = 0.8, stroke = TRUE, 
                          weight = c((rbind_df_1$i) + 5), color = 
                            c(pal(rbind_df_1$i)), group = (as.character
                                                           (rbind_df_1$route))) %>%
    leaflet::addPolygons(data = grid_tot,color = "#636363", #opacity = 0.8,weight = c((rbind_df_1$i)+3)
                group = "Concentrations", weight = 2, smoothFactor = 0.1, opacity 
                = 0.1, fillOpacity = 0.5, fillColor = grid_tot$color,) %>%
    leaflet::addTiles() %>%
    leaflet::addControl(title, position = "topleft", className = "map-title")%>%
    leaflet::addLegend(data = grid_tot, position = "bottomleft", pal = palfac, 
                       values = grid_tot$category, title = paste("US AQI Level", 
                                                                 pollutant, units, 
                                                                 sep = " ")) %>%
